    ...
    def getValue(self, driver: Driver, ts : SymbolTable):
        ...
        if type(exp_value) == str:
            if self.t.type == Types.INT64:
                ...
                driver.agregarError(f'Solo se admiten los valores enteros en el argumento de parse', self.line, self.column)

        else:
            driver.agregarError(f'Solo se admiten los valores string en la expresion de la funcion Parse', self.line, self.column)
